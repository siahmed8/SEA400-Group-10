...
def constructBayesNet(gameState: hunters.GameState):
    ...
    maxObs = maxDist + MAX_NOISE
    obsVals = list(range(maxObs + 1))

    variableDomainsDict[OBS0] = obsVals
    variableDomainsDict[OBS1] = obsVals
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        fac = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for var in eliminationOrder:
            fac, joinedFactor = joinFactorsByVariable(fac, var)
            ...
            fac.append(reducedFactor)
        
        factor = joinFactors(fac)
